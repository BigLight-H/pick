...
func (p *PickController) Lists() {
	domain, _ := config.String("source_2")
	num, _ := config.String("source_num")
	service.WhereGo(domain, util.StrToInt(num))
		...
		if isExist == int64(0) {
			...
			if id > int64(0) && bId > 0 {
				...
				if num > int64(0) {
					...
					util.MKdirs(bookid)
					//下载封面图片到目录
					service.DownloadJpg(v["image"], bookid+"/"+bookid+"_thumb.jpg", false, rootId)
				}
			...
			if created, cid, err := c.ReadOrCreate(&chapter, "Link"); err == nil {
				...
				if created {
					...
					if num > int64(0) {
						...
						//下载章节首张图片
						service.DownloadJpg(fImg, imgRole, false, rootId)
						if s["imgs"] != "" {
							go service.DoWork(bookid+"/"+epid, s["imgs"], bookid, epid, s["link"], caches, rootId)
						}
					}
				} else {
					//修改章节最后更新时间
